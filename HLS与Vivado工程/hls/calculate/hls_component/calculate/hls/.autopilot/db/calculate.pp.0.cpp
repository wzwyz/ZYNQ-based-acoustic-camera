...
# 2 "<built-in>" 2
# 1 "../src/calculate.cpp" 2
# 1 "../src/calculate.h" 1


# 1 "D:/Xilinx/Vitis_HLS/2024.1/common/technology/autopilot\\hls_stream.h" 1
# 12 "D:/Xilinx/Vitis_HLS/2024.1/common/technology/autopilot\\hls_stream.h"
# 1 "D:/Xilinx/Vitis_HLS/2024.1/common/technology/autopilot/hls_stream_39.h" 1
# 23 "D:/Xilinx/Vitis_HLS/2024.1/common/technology/autopilot/hls_stream_39.h"
namespace hls {
# 49 "D:/Xilinx/Vitis_HLS/2024.1/common/technology/autopilot/hls_stream_39.h"
...
# 13 "D:/Xilinx/Vitis_HLS/2024.1/common/technology/autopilot\\hls_stream.h" 2
# 4 "../src/calculate.h" 2
...
ap_int<24 +4> calculate_add(ap_uint<24*16> data);
ap_uint<24 +4 -1> calculate_abs(ap_int<24 +4> data);
ap_uint<2*(24 +4 -1)> calculate_square(ap_uint<24 +4 -1> data);
ap_int<2*(24 +4 -1)+1> calculate_sub(ap_uint<2*(24 +4 -1)> data0, ap_uint<2*(24 +4 -1)> data1);
__attribute__((sdx_kernel("calculate", 0))) ap_int<2*(24 +4 -1)+1> calculate(ap_uint<24*16> data_new, ap_uint<24*16> data_old);
# 2 "../src/calculate.cpp" 2

__attribute__((sdx_kernel("calculate", 0))) ap_int<2*(24 +4 -1)+1> calculate(ap_uint<24*16> data_new, ap_uint<24*16> data_old)
{
#line 1 "directive"
#pragma HLSDIRECTIVE TOP name=calculate
# 4 "../src/calculate.cpp"

#pragma HLS PIPELINE
#pragma HLS INTERFACE mode=ap_ctrl_none port=return

 ap_int<24 +4> add_new = calculate_add(data_new);
    ap_int<24 +4> add_old = calculate_add(data_old);

    ap_uint<24 +4 -1> abs_new = calculate_abs(add_new);
    ap_uint<24 +4 -1> abs_old = calculate_abs(add_old);

    ap_uint<2*(24 +4 -1)> square_new = calculate_square(abs_new);
    ap_uint<2*(24 +4 -1)> square_old = calculate_square(abs_old);

    ap_int<2*(24 +4 -1)+1> sub_new_old = calculate_sub(square_new, square_old);
# 26 "../src/calculate.cpp"
    return sub_new_old;
}
...
#pragma HLS UNROLL
 mic[i] = data.range(24 * i + 24 - 1, 24 * i);

    ap_int<24 +1> add01 = mic[0] + mic[1];
    ap_int<24 +1> add23 = mic[2] + mic[3];
    ap_int<24 +1> add45 = mic[4] + mic[5];
    ap_int<24 +1> add67 = mic[6] + mic[7];
    ap_int<24 +1> add89 = mic[8] + mic[9];
    ap_int<24 +1> addAB = mic[10] + mic[11];
    ap_int<24 +1> addCD = mic[12] + mic[13];
    ap_int<24 +1> addEF = mic[14] + mic[15];

    ap_int<24 +2> add03 = add01 + add23;
    ap_int<24 +2> add47 = add45 + add67;
    ap_int<24 +2> add8B = add89 + addAB;
    ap_int<24 +2> addCF = addCD + addEF;

    ap_int<24 +3> add07 = add03 + add47;
    ap_int<24 +3> add8F = add8B + addCF;

    ap_int<24 +4> add0F = add07 + add8F;
...
ap_uint<2*(24 +4 -1)> calculate_square(ap_uint<24 +4 -1> data)
{
    ...
    ap_uint<2*(24 +4 -1)> square = h2_hm_m2_hl_l2_ml;
    return square;
}

ap_int<2*(24 +4 -1)+1> calculate_sub(ap_uint<2*(24 +4 -1)> data0, ap_uint<2*(24 +4 -1)> data1)
{
    ap_int<2*(24 +4 -1)+1> sub = ap_int<2*(24 +4 -1)+1>(data0) - ap_int<2*(24 +4 -1)+1>(data1);
    return sub;
}
