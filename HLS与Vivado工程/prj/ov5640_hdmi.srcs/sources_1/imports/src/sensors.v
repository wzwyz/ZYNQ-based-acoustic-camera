...
module sensors #(
    ...
    input           Aclk,       // 系统时钟，用于捕获输出有效上升沿
    input           mclk,       // 传感器用时钟
    input           rst,        // 复位信号
    input [NUM-1:0] aud_adcdat, // 麦克风输出串行数据

    output                  aud_bclk,      // 麦克风通信时钟，根据data sheet，频率是采样率的64倍
    output                  aud_lrc,       // 麦克风左右声道时钟，频率与采样率相同，当其拉高时表明输出数据是左声道，拉低时则是右声道。
    output reg              rx_done_edge,  // 输出有效标识位
    output reg [WD*NUM-1:0] adc_data       // 不考虑延迟的声音信号
);
    // 实例化分频模块，生成采样时钟和信道时钟
    divider u_divider (
        .clk_in (mclk),
    ...
    always @(posedge aud_bclk or posedge rst) begin
        if (rst) rx_cnt <= 0;
        else if (lrc_edge) rx_cnt <= 0;
        else if (rx_cnt == WD) rx_cnt <= rx_cnt;
    ...
    genvar i;
    generate
        for (i = 0; i < NUM; i = i + 1) begin : gen_sensor
            // 将串行数据并行化
            always @(posedge aud_bclk or posedge rst) begin
                if (rst) adc_data[i*WD+:WD] <= 0;
                else if (rx_cnt < WD) adc_data[i*WD+WD-1-rx_cnt] <= aud_adcdat[i];
                else adc_data[i*WD+:WD] <= adc_data[i*WD+:WD];
            end
        end
    endgenerate
    ...
    always @(posedge aud_bclk or posedge rst) begin
        if (rst) rx_done <= 1'b0;
        else if (rx_cnt == WD) rx_done <= 1'b1;
        else rx_done <= 1'b0;
    end
    always @(posedge Aclk) begin
        rx_done1     <= rx_done;
        rx_done2     <= rx_done1;
        rx_done_edge <= rx_done1 && !rx_done2;
    end
endmodule
