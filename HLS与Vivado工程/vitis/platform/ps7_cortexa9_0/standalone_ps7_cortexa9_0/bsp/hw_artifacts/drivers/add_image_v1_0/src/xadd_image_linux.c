...
typedef struct {
    ...
    XAdd_image_uio_map maps[ MAX_UIO_MAPS ];
} XAdd_image_uio_info;

/***************** Variable Definitions **************************************/
static XAdd_image_uio_info uio_info;
...
static int uio_info_read_name(XAdd_image_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XAdd_image_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XAdd_image_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XAdd_image_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XAdd_image_Initialize(XAdd_image *InstancePtr, const char* InstanceName) {
	XAdd_image_uio_info *InfoPtr = &uio_info;
