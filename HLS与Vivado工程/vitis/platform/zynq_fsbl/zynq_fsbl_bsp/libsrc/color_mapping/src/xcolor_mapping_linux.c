...
typedef struct {
    ...
    XColor_mapping_uio_map maps[ MAX_UIO_MAPS ];
} XColor_mapping_uio_info;

/***************** Variable Definitions **************************************/
static XColor_mapping_uio_info uio_info;
...
static int uio_info_read_name(XColor_mapping_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XColor_mapping_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XColor_mapping_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XColor_mapping_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XColor_mapping_Initialize(XColor_mapping *InstancePtr, const char* InstanceName) {
	XColor_mapping_uio_info *InfoPtr = &uio_info;
